mut_bar = (function () {
  ...
  p._set_xw = function () {
    ...
    var x = "x",
      y = "y";
    var padding1 = that.padding.left,
      padding2 = this.padding.top;
    var plot1 = that.plot.w,
      plot2 = this.plot.h;
        ...
        .attr("transform", function (d, i) {
          ...
          if (that.options.grid_xs[idx].text_anchor_ext == true) {
            ...
            if (that.options.grid_xs[idx].orient == "bottom") {
              tx =
                tx +
                p._xy_position_text(that.asc, that.plot.w, that.padding.left, x_items, classes[1], that.options.zoom, sift, 0);
              ty = ty + that.padding.top + that.plot.h + f;
            } else if (that.options.grid_xs[idx].orient == "top") {
              tx =
                tx +
              ...
              ty =
                ty +
                p._xy_position_text(that.asc, that.plot.h, that.padding.top, x_items, classes[1], that.options.zoom, sift, 0);
            } else if (that.options.grid_xs[idx].orient == "right") {
              tx = tx + that.padding.left + that.plot.w + f;
              ty =
                ty +
  ...
  p._set_w_options = function () {
    ...
    if (this.options.grid_y != 0) {
      ...
      this.svg_obj.selectAll("g.axis").attr("transform", trans).call(this.y_axis.tickSize(ticksize));
    }
    ...
    if (this.options.titles.length > 0) {
      ...
      for (var i = 0; i < this.options.titles.length; i++) {
        trans_title.push(
          this._title_pos(
            this.options.titles[i].wide,
            this.options.titles[i].orient,
            this.options.titles[i].text_anchor,
            this.options.titles[i].text_rotate,
            this.options.titles[i].sift_x,
            this.options.titles[i].sift_y,
            this.svg,
            this.plot,
            this.padding,
  ...
  p._set_yh = function () {
    ...
    // transparent-bar
    this.svg_obj.selectAll("g.transparent_bar").selectAll("rect").attr(height, plot1);
    ...
    if (this.options.grid_y != 0) {
      ...
      this.svg_obj.selectAll("g.axis").call(this.y_axis.scale(y_scale).tickSize(tick_size));
    ...
    if (orient == "left") {
      ...
      if (String(rotate) == "90") {
        ...
        if (anchor == "end") {
          ...
          y = y + Math.floor(plot.h / 2);
        }
      } else if (String(rotate) == "-90") {
        // 2
        x = x + Math.floor(wide / 2);
  ...
  p.update = function () {
    ...
    // transparent-bar
    this.svg_obj.selectAll("g.transparent_bar").selectAll("rect").remove();
    ...
    if (this.options.grid_xs.length > 0) {
      ...
      // label
      this.svg_obj.selectAll("g.label_x").selectAll("text").remove();
      ...
      // grid
      this.svg_obj.selectAll("g.grid_x").selectAll("line").remove();
  ...
  p._init = function () {
    ...
    for (var idx2 = 0; idx2 < this.dataset.length; idx2++) {
      this.svg_obj.append("g").attr("class", this.dataset[idx2].name).selectAll("rect").data([]).enter();
    }

    // transparent-bar
    this.svg_obj.append("g").attr("class", "transparent_bar").selectAll("rect").data([]).enter();
    ...
    // x-axis
    if (this.options.grid_xs.length > 0) {
      this.svg_obj.append("g").attr("class", "label_x").selectAll("line").data([]).enter();

      this.svg_obj.append("g").attr("class", "grid_x").selectAll("line").data([]).enter();
    }
    ...
    if (this.options.grid_y != 0) {
      ...
      this.svg_obj
        ...
        .call(this.y_axis.orient(this.options.grid_y.orient).ticks(this.options.grid_y.ticks));
    }
  ...
  p.reset_select = function () {
    this.svg_obj.selectAll("g.transparent_bar").selectAll("rect").style("opacity", 0);
  };
  ...
  p.brush_reset = function () {
    ...
    this.options.brush.rect.clear().event(d3.select(".brush"));
    this.svg_obj.selectAll("rect.extent").attr("width", "0").attr("x", "0");
  };
